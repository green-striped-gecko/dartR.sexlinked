...
gl.infer.sex <- function(gl_sexlinked,
  ...
  obj <- list(gl1, gl2, gl3)
  n_ind  <- NULL
  for (x in obj) {
    if (!is.null(x)) {
      n_ind <- length(x@ind.names)
      break
    }
  }
  ...
  if (!is.null(gl1)) {
    ...
    message(warn(
      "  No W-linked/Y-linked loci were found. Assigning NAs to
  ...
  if (!is.null(gl2) && gl2@n.loc >= 2) {
    ...
    message(warn(
      "  Not enough Z-linked/X-linked loci (at least 2 are needed). Assigning NAs to
  ...
  Fun <- function(x, y, z) {
    d  <- c(x, y, z)
    yy <- data.frame(x = c("F", "M"), freq = c(sum(d == "F", na.rm = TRUE),
                                               sum(d == "M", na.rm = TRUE)))
    yy <- yy[order(-yy$freq), ]
    value <- if (length(unique(na.omit(d))) == 1)
      unique(na.omit(d))
    else
      sprintf('*%s', yy[1, 1])
    return(value)
...
Z.sex <- function(gl, system = NULL, seed = 42) {
  ...
  Z_unclean <- t(apply(data.frame(n0.z, n1.z), 1, function(x)
    x / sum(x)))
  Z <- na.omit(Z_unclean)
...
g.sex <-  function(gl,
  ...
  Z_unclean <- t(apply(data.frame(n0.g, n1.g), 1, function(x)
    x / sum(x)))
  Z <- na.omit(Z_unclean)
  Zna <- Z_unclean[rowSums(is.na(Z_unclean)) > 0, ]
